...
async function cleanupAndRecreateTable() {
  try {
    console.log('🔄 Cleaning up and recreating pending_products table...');
    console.log('Using OWNER credentials for ep-holy-leaf-at8ruz1r\n');

    // Step 0: Clean up any existing temporary tables
    console.log('0. Cleaning up existing temporary tables...');
    try {
      await sql`DROP TABLE IF EXISTS pending_products_new`;
    ...
    if (newConstraint.length > 0) {
      ...
      if (newFormat === targetFormat) {
        ...
        console.log('Length difference:', newFormat.length, 'vs', targetFormat.length);
      }
    }
