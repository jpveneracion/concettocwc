...
async function checkPgConstraintFormat() {
  console.log('🔍 CHECKING CONSTRAINT FORMAT FROM pg_catalog');
    ...
    try {
      const constraint = await sql`
        SELECT
          ...
          pgc.consrc AS constraint_source,
          pgc.conbin AS constraint_binary,
      ...
      if (constraint.length > 0) {
        console.log(`\n${db.name} constraint details:`);
        console.log(`Constraint Name: ${constraint[0].constraint_name}`);
        console.log(`Constraint Source: ${constraint[0].constraint_source}`);
        console.log(`Constraint Binary: ${constraint[0].constraint_binary}`);
        console.log(`Constraint Definition: ${constraint[0].constraint_definition}`);
      } else {
