...
// Run with: npm run db:migrate-company-product
// This script applies the company_product_definitions table to Database 1
...
async function runMigration(): Promise<void> {
  console.log('🔄 Starting Company Product System migration...');
  ...
  try {
    ...
    console.log('📋 Creating unique index on company_id and code...');
...
async function verifyMigration(sql: any): Promise<void> {
  ...
  const comments: any[] = await sql`
    SELECT
      obj_description('company_product_definitions'::regclass, 'pg_class') as table_comment,
      col_description('company_product_definitions'::regclass, 'is_approved_for_global') as approval_comment,
      col_description('company_product_definitions'::regclass, 'global_product_id') as product_comment
  `;

  if (comments[0].table_comment) {
    console.log(`✅ Table comment: "${comments[0].table_comment}"`);
  } else {
    console.log('⚠️  Table comment missing');
  }

  if (comments[0].approval_comment) {
    console.log(`✅ Column comment (is_approved_for_global): "${comments[0].approval_comment}"`);
  } else {
    console.log('⚠️  Column comment missing for is_approved_for_global');
  }

  if (comments[0].product_comment) {
    console.log(`✅ Column comment (global_product_id): "${comments[0].product_comment}"`);
  } else {
