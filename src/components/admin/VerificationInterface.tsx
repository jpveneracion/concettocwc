...
export default function VerificationInterface({
  ...
  function getAutomaticVerificationStatus(status?: string) {
    ...
    const statusConfig = {
      matched: { bgColor: 'bg-emerald-50', textColor: 'text-emerald-700', borderColor: 'border-emerald-200', label: 'Matched' },
      no_webhook_data: { bgColor: 'bg-stone-50', textColor: 'text-stone-700', borderColor: 'border-stone-200', label: 'No Webhook Data' },
      amount_mismatch: { bgColor: 'bg-rose-50', textColor: 'text-rose-700', borderColor: 'border-rose-200', label: 'Amount Mismatch' },
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-8">
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <h2 className="text-xl font-semibold text-stone-900 mb-2">Loading Verification Details</h2>
          <p className="text-stone-600 text-center">Please wait while we fetch the verification information...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-8">
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <h2 className="text-xl font-semibold text-stone-900 mb-2">Error Loading Verification</h2>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-indigo-100 text-sm">Manual review for verification: {verificationId.slice(0, 8)}...</p>
            </div>
          </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-stone-900 mb-4 flex items-center gap-2">
            <span className="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center"><ClipboardList className="w-4 h-4 text-indigo-600" /></span>
            Verification Details
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Reference Number */}
            <div className="bg-stone-50 rounded-lg p-4 border border-stone-200">
              <h3 className="text-sm font-medium text-stone-700 mb-2">Reference Number</h3>
            ...
            {/* Submitted Time */}
            <div className="bg-stone-50 rounded-lg p-4 border border-stone-200">
              <h3 className="text-sm font-medium text-stone-700 mb-2">Submitted At</h3>
              <p className="text-lg text-stone-900">{formatDate(verification.submitted_at)}</p>
            </div>

            {/* Verification Method */}
            <div className="bg-stone-50 rounded-lg p-4 border border-stone-200">
              <h3 className="text-sm font-medium text-stone-700 mb-2">Verification Method</h3>
            ...
            <div className="bg-stone-50 rounded-lg p-4 border border-stone-200">
              <h3 className="text-sm font-medium text-stone-700 mb-2">Automatic Status</h3>
              <div className="flex items-center gap-2">
                {verification.automatic_verification_attempted ? (
                  getAutomaticVerificationStatus(verification.automatic_verification_status)
                ) : (
                  <span className="text-stone-400 text-sm">Not attempted</span>
                )}
              </div>
            </div>

            {/* User Information */}
            <div className="bg-gradient-to-br from-indigo-50 to-indigo-50 rounded-lg p-4 border border-indigo-200">
              <h3 className="text-sm font-medium text-stone-700 mb-2">User Information</h3>
              <div className="space-y-1">
                <p className="text-stone-900 font-medium">{verification.user_name || 'Unknown'}</p>
                <p className="text-stone-600 text-sm">{verification.user_email || 'No email'}</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
              ...
              <div className="space-y-1">
                ...
                <p className="text-stone-900 font-bold text-lg">{formatCurrency(verification.plan_amount)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {webhookData && (
          <div>
            <h2 className="text-lg font-semibold text-stone-900 mb-4 flex items-center gap-2">
              <span className="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center"><Radio className="w-4 h-4 text-indigo-600" /></span>
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-cyan-50 rounded-lg p-4 border border-indigo-200">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Transaction Number */}
                <div>
                  <h3 className="text-sm font-medium text-stone-700 mb-2">Transaction Number</h3>
                  <p className="text-lg font-mono font-bold text-stone-900">{webhookData.transaction_number}</p>
                  {webhookData.cleaned_transaction_number !== webhookData.transaction_number && (
                    <p className="text-sm text-stone-600">Cleaned: {webhookData.cleaned_transaction_number}</p>
                  )}
                </div>

                {/* Amount */}
                <div>
                  <h3 className="text-sm font-medium text-stone-700 mb-2">Amount</h3>
                  <p className="text-lg font-bold text-stone-900">{formatCurrency(webhookData.amount)}</p>
                ...
                {/* Sender Information */}
                <div>
                  <h3 className="text-sm font-medium text-stone-700 mb-2">Sender Information</h3>
                  <p className="text-stone-900 font-medium">{webhookData.sender_name || 'N/A'}</p>
                  <p className="text-stone-600 text-sm font-mono">{webhookData.sender_account || 'N/A'}</p>
                </div>

                {/* Transaction Time */}
                <div>
                  <h3 className="text-sm font-medium text-stone-700 mb-2">Transaction Time</h3>
                  <p className="text-stone-900">{formatDate(webhookData.transaction_time)}</p>
                  <p className="text-stone-600 text-sm">Received: {formatDate(webhookData.received_at)}</p>
                </div>
              </div>
              ...
              {webhookData.notification_text && (
                <div className="mt-4 pt-4 border-t border-indigo-200">
                  <h3 className="text-sm font-medium text-stone-700 mb-2">Notification Text</h3>
                  <p className="text-sm text-stone-600 bg-white p-3 rounded border border-stone-200 font-mono">
                    {webhookData.notification_text}
                  </p>
                </div>
              )}
        ...
        <div>
          ...
          <div className="bg-stone-50 rounded-lg p-4 border border-stone-200">
            ...
            <div className="bg-white rounded-lg border border-stone-300 overflow-hidden flex justify-center">
              <img
        ...
        {verification.notes && (
          <div>
            <h2 className="text-lg font-semibold text-stone-900 mb-4 flex items-center gap-2">
              <span className="w-8 h-8 bg-stone-100 rounded-lg flex items-center justify-center"><NotebookPen className="w-4 h-4 text-stone-600" /></span>
              User Notes
            </h2>
            <div className="bg-stone-100 rounded-lg p-4 border border-stone-300">
              <p className="text-stone-700 whitespace-pre-wrap">{verification.notes}</p>
            </div>
          </div>
        )}
        ...
        {isPending && (
          <div>
            <h2 className="text-lg font-semibold text-stone-900 mb-4 flex items-center gap-2">
              <span className="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center"><Scale className="w-4 h-4 text-indigo-600" /></span>
              Admin Actions
            </h2>
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-stone-700 mb-2">
                Admin Notes <span className="text-stone-500 font-normal">(Optional)</span>
              </label>
              <textarea
                value={adminNotes}
        ...
        {!isPending && verification.admin_notes && (
          <div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-stone-700 whitespace-pre-wrap">{verification.admin_notes}</p>
            </div>
          </div>
        )}
      </div>
