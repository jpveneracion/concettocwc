  ...
  if (plans.length === 0) {
    return (
      <div className="bg-white border border-stone-200 rounded-lg p-12 text-center">
        <CreditCard className="w-14 h-14 mx-auto mb-4 text-stone-300" />
        <h3 className="text-lg font-semibold text-stone-900 mb-2">No Subscription Plans</h3>
      ...
      {plans.map((plan) => (
        <div
          ...
          className={`bg-white border rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden ${
            plan.is_active ? 'border-stone-200' : 'border-stone-300 opacity-75'
          }`}
        >
          {/* Plan Header */}
          <div className={`p-6 border-b ${
            plan.is_active ? 'border-stone-200 bg-white' : 'border-stone-300 bg-stone-50'
          }`}>
            ...
            <div className="flex items-baseline gap-1 mb-2">
              ...
              <span className="text-stone-600">/ {plan.interval}</span>
            </div>
          ...
          <div className="p-6">
            ...
            {/* Features Preview */}
            <div className="mb-4">
              <h4 className="text-xs font-semibold text-stone-700 mb-2 uppercase tracking-wide">
                Key Features
              </h4>
            ...
            <div className="space-y-2 pt-4 border-t border-stone-200">
              <button
                ...
                className="w-full bg-indigo-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-indigo-700 disabled:bg-stone-400 disabled:cursor-not-allowed min-h-[44px] text-sm"
              >
                Edit Plan
              </button>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  disabled={togglingPlan === plan.id || deletingPlan === plan.id}
                  className="bg-red-100 text-red-800 px-4 py-2 rounded-lg font-medium hover:bg-red-200 disabled:bg-stone-100 disabled:cursor-not-allowed min-h-[44px] text-sm"
                >
