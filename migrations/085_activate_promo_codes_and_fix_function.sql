-- Migration 085: Activate existing promo codes and fix plan matching in function

BEGIN;

-- Activate existing promo codes
UPDATE activation_codes SET is_active = true WHERE is_active = false;

-- Fix the function - fix jsonb @> text[] issue and plan matching
...
BEGIN
  ...
  -- Also check for period names (monthly, quarterly, annual)
  IF p_plan IN ('monthly', 'quarterly', 'annual', 'MONTHLY', 'QUARTERLY', 'ANNUAL') THEN
    v_resolved_plan := upper(p_plan);
  END IF;
  ...
  -- Check if code applies to requested plan - handle both array formats
  -- applicable_plans is jsonb array like ["monthly", "quarterly"] or ["BASIC", "PRO"]
  IF NOT (v_activation_code.applicable_plans ?| ARRAY[v_resolved_plan]) THEN
