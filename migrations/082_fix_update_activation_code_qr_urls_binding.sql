-- Migration 082: Fix dynamic SQL parameter binding in update_activation_code_qr_urls
...
BEGIN
  ...
  IF p_usage_limit IS NOT NULL THEN
    ...
    update_values := array_append(update_values, p_usage_limit::text);
    param_index := param_index + 1;
  END IF;

  -- Execute dynamic update if fields provided
  IF array_length(update_fields, 1) > 0 THEN
    -- Add the promo_code_id as the last parameter
    ...
    EXECUTE v_sql USING update_values INTO v_result;
  END IF;
