-- Fix test function to match actual PostgreSQL patterns
...
BEGIN
  ...
  BEGIN
    ...
    IF EXISTS (
      ...
      AND (qual LIKE '%SELECT quotes.id FROM quotes%' OR with_check LIKE '%SELECT quotes.id FROM quotes%')
      AND (qual LIKE '%quotes.company_id = get_current_company_id()%' OR with_check LIKE '%quotes.company_id = get_current_company_id()%')
    ) THEN
  ...
  BEGIN
    ...
    IF EXISTS (
      ...
      AND (qual LIKE '%SELECT quote_items_1.supplier_cost_sqft FROM quote_items quote_items_1%' OR
           qual LIKE '%SELECT quote_items_1.supplier_amount FROM quote_items quote_items_1%' OR
           with_check LIKE '%SELECT quote_items_1.supplier_cost_sqft FROM quote_items quote_items_1%' OR
           with_check LIKE '%SELECT quote_items_1.supplier_amount FROM quote_items quote_items_1%')
    ) THEN
