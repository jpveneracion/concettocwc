...
async function exploreDatabase() {
  ...
  try {
    ...
    // 6. Check if our RLS functions are installed
    console.log('🔍 RLS FOUNDATION CHECK:');
    try {
      const rlsTest = await sql`SELECT test_rls_foundation() as result`;
      console.log('  ✅ RLS foundation functions installed:', rlsTest[0].result);
    } catch (err) {
      console.log('  ❌ RLS foundation functions not installed:', err.message);
    }
